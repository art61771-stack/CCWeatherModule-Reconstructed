...
checks=[
 ...
 'Version: 1.2.0' in (root/'control').read_text(), 'Icon: https://' in (root/'control').read_text(),
]
info=plistlib.loads((root/'Resources/Info.plist').read_bytes()); checks.extend([info['CFBundleVersion']=='120',info['CFBundleShortVersionString']=='1.2.0'])
...
assert (root/'src/CYCaiyunProvider.m').read_bytes()==(root/'evidence/caiyun120/CYCaiyunProvider.m').read_bytes()
print(f'PASS {len(checks)} source120 static assertions + component copy identity (NOT compile/runtime)')
